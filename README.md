...
- the [OpenCADC TAP library (youcat)]((https://github.com/opencadc/tap/tree/main/youcat))
...
### Using the OpenCADC TAP Library (youcat)
...
`youcat` can of course run as a standalone TAP service, but for this use case `youcat` is using the `rucio` tables created by the `dachs` installation.


#### Backend Database

`youcat` uses the `postgres-metadata database`, and initializies it with:

- A user, `tapadm`, to create and manage all the tables.
- A user, `tapuser`, for executing queries submitted using the TAP API.
- A schema, `tap_schema`, containing the necessary TAP metadata tables (tapadm user must have authorization to create objects).
- A schema, `tap_upload`, for user(tapuser user must have authorization to create tables).
- A schema, `uws`,  (tapadm user must have authorization to create objects).

| Service           | Port | User         | Password         | Other credentials     |
|-------------------|------|----------- --|------------------|-----------------------|
| postgres-metadata | 5432 | postgres     | secret           | database=metadata     |
|                   |      | tapadm       | secret           | database=metadata     |
|                   |      | tapuser      | secret           | database=metadata     |
...
For SKA, `youcat` is configured to use the SKA IAM prototype for authentication. All calls to `youcat` require authentication. To authenticate using a curl command include a header with the callers bearer token:

--header "authorization: bearer $SKA_IAM_TOKEN"

`youcat` can optionally be configured with an admin user, which grants permissions to that user to create schemas and tables in the tap_schema, and ingest external tables into the tap_schema. A configured admin user is required to ingest the `rucio.obscore` tables into `youcat`, as outlined in the instructions below.
...
|-------------------|-------------------------------------------|-----------------------------------------------------------------------------------|
| youcat            | http://localhost:8080/youcat/sync         | synchronous TAP queries                                                           |
|                   | http://localhost:8080/youcat/async        | asynchronous TAP queries                                                          |
|                   | http://localhost:8080/youcat/tables       | view table metadata                                                               |
|                   | http://localhost:8080/youcat/table-update | - add an index to a column in the tap_schema and create the index in the database |
|                   |                                           | - add the metadata for an existing table or view to the tap_schema                |
|                   | http://localhost:8080/youcat/load         | stream data to a table                                                            |
|                   | http://localhost:8080/youcat/permissions  | manage permissions on a schema or table in the tap_schema                         |
|                   | http://localhost:8080/youcat/capabilities | VOSI capabilities document                                                        |
|                   | http://localhost:8080/youcat/availability | VOSI availability document                                                        |


#### Deployment

To run `youcat` locally, use the provided `docker-compose` file. Since `youcat` uses `ObsCore` tables created by `dachs`, both the `postgres-metadata` and `dachs` containers must be available before running `youcat`.
...
##### create an ivoa.obscore view on the rucio.obscore table
...
##### add the ivoa and rucio schemas to the youcat tap_schema
To ingest the rucio.obscore table into the tap_schema as a ivoa.obscore view, the ivoa and rucio schemas must exist in the tap_schema. They can be added to `youcat` using the `\tables` endpoint, which enables adding and updating of tap_schema schemas, tables, and columns.
...
Add the rucio schema to the tap_schema:
```bash
curl -v \
--header "authorization: bearer $SKA_IAM_TOKEN" \
--header "Content-Type: application/x-vosi-schema" \
--header "x-schema-owner: openid https://ska-iam.stfc.ac.uk/ $CLIENT_ID" \
--data "@youcat/rucio-schema-desc.xml" \
-X PUT http://localhost:9090/youcat/tables/rucio
```
...
<uws:job xmlns:uws="http://www.ivoa.net/xml/UWS/v1.0" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
  ...
  <uws:runId />
  <uws:ownerId>jburke</uws:ownerId>
...
##### get the jobID from the document and export the jobID:

```bash
export JOB_ID="tr26z4dk1fmu41jz"
```

##### run the job by setting the job phase to run
...
--header "authorization: bearer $SKA_IAM_TOKEN" \
--data "phase=run" \
http://localhost.cadc.dao.nrc.ca:9090/youcat/table-update/$JOB_ID/phase
```

##### check the job phase until PHASE = COMPLETED
...
curl -v \
--header "authorization: bearer $SKA_IAM_TOKEN" \
http://localhost.cadc.dao.nrc.ca:9090/youcat/table-update/$JOB_ID
```
returns
```
<uws:job xmlns:uws="http://www.ivoa.net/xml/UWS/v1.0" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
...
Test the ingest by getting the VOSI-tables document for the ivoa.obscore view;

```bash
curl -v \
--header "authorization: bearer $SKA_IAM_TOKEN" \
http://localhost.cadc.dao.nrc.ca:9090/youcat/tables/ivoa.obscore
```
...
##### augment the tap_schema metadata for the view
...
You can query the TAP endpoint at [http://localhost:8080/youcat/sync](http://localhost:8080/youcat/sync) toget the contents of the `ivoa.obscore` view along with the updated view metadata.
